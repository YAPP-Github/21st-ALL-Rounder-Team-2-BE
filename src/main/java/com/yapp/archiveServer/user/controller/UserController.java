...
import static com.yapp.archiveServer.global.exception.ErrorCode.USER_ALREADY_EXISTS;
import static com.yapp.archiveServer.global.exception.ErrorCode.USER_FORBIDDEN;
...
public class UserController {
  ...
  public ResponseEntity<CreateUserResponseDto> createUserByUid(HttpServletRequest request,
    ...
    if (!request.getAttribute("uid").equals(uid)) {
      new CustomException(USER_FORBIDDEN, "uid가 일치하지 않습니다" + uid);
    }

    userService.findByUid(uid)
        .ifPresent(existedUser -> {
          throw new CustomException(USER_ALREADY_EXISTS, "user UID = " + uid);
        });
